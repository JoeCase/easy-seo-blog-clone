...
<form method="post" action="<?php echo url(array('action' => 'saveText')); ?>" class="text">
	<p class="wide">
		...
		<textarea id="text-content" name="content"><?php echo (isset($post->content) ? str_replace('&', '&amp;', $post->content) : ''); ?></textarea>
		<input class="inputbar" type="text" name="tags" value="<?php echo (isset($post->tags) ? implode(',', objValues($post->tags, 'name')) : ''); ?>"/>
		<input id="input-main-link" type="hidden" name="mainLink" value=""/>	
	...
	<p class="clearfix pcb">
		<span class="post-cancel-buttons">
			<?php if (!isset($post->published)): ?>
				<button type="submit" name="published" value="1" class="btn post-text" <?php echo isset($options['posts_left']) && $options['posts_left'] <= 0 ? 'disabled="disabled"' : ''?>><?php echo isset($post->published) && $post->published ? __('button_update') : __('button_publish'); ?></button>
			...
			<?php else: ?>
				...
				<button type="submit" name="published" value="1" class="btn post-text"><?php echo isset($post->published) && $post->published ? __('button_update') : __('button_publish'); ?></button>
				<?php else: ?>
				<button type="submit" name="published" value="0" class="btn default save-text"><?php _e('button_save'); ?></button>
				<?php endif; ?>
			<?php endif; ?>
			<button type="button" value="cancel" class="btn default js-post-text-cancel"><?php _e('button_cancel'); ?></button>			
		</span>		
	</p>
</form>