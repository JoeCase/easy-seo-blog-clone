...
<form method="post" action="<?php echo url(array('action' => 'saveVideo')); ?>" class="video">
	<p class="wide">
		...
		<span class="url-inputbar">
			<span class="inputbar-wrap" style="padding-right: 140px;">
				<input class="inputbar" type="text" placeholder="<?php _e('video_url'); ?>" name="url" value="<?php echo (isset($post->url) ? $post->url: ''); ?>"/>
			</span>
			<span class="url-barbuttons">
				<a class="btn default js-video-filepicker <?php echo $options['is_premium'] ? '' : 'premium-features'; ?>"><?php _e('social_network'); ?></a>
			</span>
		</span>
		<span class="thumbnail center video-preview hidden"></span>
		<textarea id="video-description" name="content"><?php echo (isset($post->content) ? str_replace('&', '&amp;', $post->content) : ''); ?></textarea>
	...
	<p class="clearfix pcb">
		<span class="post-cancel-buttons">
			<?php if (!isset($post->published)): ?>
				<button type="submit" name="published" value="1" class="btn post-video" <?php echo isset($options['posts_left']) && $options['posts_left'] <= 0 ? 'disabled="disabled"' : ''?>><?php echo isset($post->published) && $post->published ? __('button_update') : __('button_publish'); ?></button>
			...
			<?php else: ?>
				<?php if ($post->published): ?>
					<button type="submit" name="published" value="1" class="btn post-video"><?php echo isset($post->published) && $post->published ? __('button_update') : __('button_publish'); ?></button>
				<?php else: ?>
					<button type="submit" name="published" value="0" class="btn default save-video"><?php _e('button_save'); ?></button>
				<?php endif; ?>
			<?php endif; ?>
			<button type="button" value="Cancel text" class="btn default js-post-video-cancel"><?php _e('button_cancel'); ?></button>
  		</span>
	</p>
</form>
