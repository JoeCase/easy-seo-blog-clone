...
<head>
	...
	<?php 
		if(isset($GLOBALS['url']['slug'])){
			...
			$meta = loadPosts($GLOBALS['current_page'], 5, true, $GLOBALS['url'], $user);
			if(!empty($meta)){
    ...
    <link type="text/css" href="<?php echo base_url(); ?>stylesheets/css/mobile.min.css?v=<?php echo $options['version']; ?>" rel="stylesheet">
	...
	<style type="text/css">
		<?php
			...
			echo '.caption p, .caption {color:'.$options['text_color_rgba'].' !important; font-size:'.min(MMFS,$options['textFontSize']).'px; font-family:'.$options['fonts'][$options['textFontFamily']]->fontFamily.';}';
			echo '.caption p a, .url a, .url p a{color:'.$options['links_color_rgba'].' !important; border-bottom-color: '.$options['links_color_rgba'].'; font-size:'.min(MMFS,$options['linksFontSize']).'px; font-family:'.$options['fonts'][$options['linksFontFamily']]->fontFamily.';}';
			echo '.title, .title > a {color:'.$options['title_color_rgba'].' !important;  font-size:'.min(MMFS,$options['titleFontSize']).'px !important; font-family:'.$options['fonts'][$options['titleFontFamily']]->fontFamily.';}';
			echo '.tag a{color:'.$options['tags_color_rgba'].' !important;  font-size:'.min(MMFS,$options['tagsFontSize']).'px; font-family:'.$options['fonts'][$options['tagsFontFamily']]->fontFamily.';}';
			echo '.datebox .date {color:'.$options['date_color_rgba'].' !important;  font-size:'.min(MMFS,$options['dateFontSize']).'px; font-family:'.$options['fonts'][$options['dateFontFamily']]->fontFamily.';}';
			echo '.pagelink.current, .pagelink, .pagelink a {color:'.$options['pagination_color_rgba'].' !important;  font-size:'.min(MMFS,$options['paginationFontSize']).'px;border-bottom-color: '.$options['pagination_color_rgba'].'; font-family:'.$options['fonts'][$options['paginationFontFamily']]->fontFamily.';}';
			echo '.back-link a {color:'.$options['btf_color_rgba'].' !important;  font-size:'.min(MMFS,$options['btfFontSize']).'px; font-family:'.$options['fonts'][$options['btfFontFamily']]->fontFamily.';}';
			echo '.tag-title {color:'.$options['dbt_color_rgba'].' !important;  font-size:'.min(MMFS,$options['dbtFontSize']).'px; font-family:'.$options['fonts'][$options['dbtFontFamily']]->fontFamily.';}';			
			echo '.read_more {color:'.$options['read_more_color_rgba'].' !important;  font-size:'.min(MMFS,$options['readMoreFontSize']).'px; font-family:'.$options['fonts'][$options['readMoreFontFamily']]->fontFamily.';}';
			
			echo '.single_post .caption p, .single_post .caption {font-size:'.min(MMFS,$options['textSingleFontSize']).'px;}';
			echo '.single_post .caption p a, .url a, .single_post .url p a{font-size:'.min(MMFS,$options['linksSingleFontSize']).'px;}';
			echo '.single_post .title, .single_post .title > a {font-size:'.min(MMFS,$options['titleSingleFontSize']).'px !important;}';
			echo '.single_post .tag a{font-size:'.min(MMFS,$options['tagsSingleFontSize']).'px;}';
			echo '.single_post .datebox .date {font-size:'.min(MMFS,$options['dateSingleFontSize']).'px;}';
			echo '.single_post .pagelink.current, .single_post .pagelink, .single_post .pagelink a {font-size:'.min(MMFS,$options['paginationSingleFontSize']).'px;}';
			echo '.single_post .back-link a {font-size:'.min(MMFS,$options['btfSingleFontSize']).'px;}';
			//echo '.single_post .tag-title {font-size:'.min(MMFS,$options['dbtSingleFontSize']).'px;}';
...
<body class="clearfix">
	<div class="clearfix allPosts mobile <?php 
		if(!isset($GLOBALS['url']['slug'])){
			echo 'columns_1';
		} else {
			echo 'single_post';
